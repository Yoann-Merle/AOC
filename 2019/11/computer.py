...
class Computer:
        ...
        def write(self, value, indice = 2):
                try:
                        ...
                        self.extendComp(self.entry[indice])
                        self.entry[self.getLocationToWrite(indice)] = value
...
def draw(wh, bl):
        maxHeight = max(bl, key= lambda x : x[1])[1] + 1
        minHeight = min(bl, key= lambda x : x[1])[1]
        maxWidth = max(bl, key= lambda x : x[0])[0] + 1
        minWidth = min(bl, key= lambda x : x[0])[0]
        line = []
        for h in reversed(range(minHeight, maxHeight)):
                line = ''
                for c in reversed(range(minWidth, maxWidth)):
                        if (c, h) in bl:
                                line += ' '
                        else:
                                line += '#'
...
while True:
        if location in paintBlack:
                ...
                if color == '':
                        break
        location, paintBlack, paintWhite = paint(location, color, paintBlack, paintWhite)
        angleRotation = comp.run('')
        location, orientation = move(location, orientation, angleRotation) 

draw(paintWhite, paintBlack)
